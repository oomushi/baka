class Message < ActiveRecord::Base
  ...
  def viewable_by? user
    reader<user.max_group or reader.eql? user.max_group
  end
  def creatable_by? user
    self.user.eql? user or
      writer <= user.max_group or
  ...
  def read_write?
    errors.add :writer_id, I18n.t(:ko_writer) if self.writer.level>self.message.writer.level and !self.message.section
